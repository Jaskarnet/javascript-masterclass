...
//       Odd of victory Borrussia Dortmund: 6.5
// Get the team names directly from the game object, don't hardcode them (except for "draw"). HINT: Note how the odds and the game objects have the same property names 😉
for (const [winningTeam, odd] of Object.entries(game.odds)) {
  const teamStr = winningTeam === 'x' ? 'draw' : `victory ${game[winningTeam]}`;
...
for ([minute, eventName] of gameEvents) {
  ...
  console.log(`[${half} HALF] ${minute}: ${eventName}`);
}
...
function listCamelCase() {
  ...
  for (const [index, row] of text.split('\n').entries()) {
    ...
    for (const [index, word] of rest.entries()) {
      rest[index] = word[0].toUpperCase() + word.slice(1);
    }
    console.log(
...
document.querySelector('button').addEventListener('click', listCamelCase);
